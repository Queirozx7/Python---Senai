...
### Função REBOOT game 
def reboot_game():
    global pontos,x_Snake,y_Snake, lista_cabeca_Snake, lista_Snake, x_Maca, y_Maca, lost , velocidade, key_DOWN, key_LEFT, key_UP, key_RIGTH
    x_Snake = largura/2
    y_Snake = largura/2
    lista_cabeca_Snake = []
    lista_Snake, x_Maca = []
    ...
    lost = False
    velocidade = 5
    key_DOWN = key_LEFT = key_UP = False
    key_RIGTH = True
...
#Informações texto de reboot (perdeu)
# utiilizei a mesma fonte da pontuação
...
while True:
    relogio.tick(25)  # padrao 60 - super rapido
    tela.fill((54,54,54))
    msg = f'Pontos: {pontos}'
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN: ## QUANDO TECLA FOR CLICADA (INFORMAR QUAL A TECLA DEPOIS)
            if event.key == K_LEFT and not(key_RIGTH):
                movimentoX = -velocidade
                movimentoY = 0
                key_UP = key_DOWN = key_RIGTH = False
                key_LEFT = True      
                
            if event.key == K_RIGHT and not(key_LEFT):
                movimentoX = velocidade
                movimentoY = 0
                key_LEFT = key_DOWN = key_UP = False
                key_RIGTH = True
                
            if event.key == K_DOWN and not(key_UP):
                movimentoY = velocidade
                movimentoX = 0
                
                key_LEFT = key_UP = key_RIGTH = False
                key_DOWN = True      
                          
            if event.key == K_UP and not(key_DOWN):
                movimentoY = - velocidade
                movimentoX = 0        
                key_LEFT = key_DOWN = key_RIGTH = False
                key_UP = True
...
### Sistema de GAME OVER
    if lista_Snake.count(lista_cabeca_Snake) > 1:
        ...
        while lost:
            tela.fill((0,0,0))
            mensagem = f'Parabéns voçê fez um total de {pontos} pontos. \nPara jogar novamente pressione a tecla R no seu teclado'
            textoreboot = fonte.render(mensagem,True, (255,255,255))
